...
from data.utils.batch_collate_fn import collate_fn
from data import preprocess as transforms
...
def main(args):
    ...
    model_ema = ModelEMA(model) if args.use_ema else None
    ...
    optimizer = optimizer.AdamW(param_groups, lr=args.lr)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        train_stats = train_one_epoch(
            ...
            model_ema=model_ema,
            args=args)
        test_stats_woema = evaluate(data_loader_val, model, device)
        print(f"W/o EMA, Accuracy of the network on the {len(dataset_val)} test images: {test_stats_woema['acc1']:.1f}%")
        max_accuracy_woema = max(max_accuracy_woema, test_stats_woema["acc1"])
        print(f'W/o EMA, Max accuracy: {max_accuracy_woema:.2f}%')
        if model_ema:
            original_weight = copy.deepcopy(model.state_dict())
            model.set_dict(model_ema.apply())
        test_stats = evaluate(data_loader_val, model, device)
        ...
        if model_ema:
            model.set_dict(original_weight) # reset original weight
            del original_weight
        ...
        if log_writer is not None:
            ...
            log_writer.add_scalar('perf/test_acc1_woema', test_stats_woema['acc1'], epoch)
            log_writer.add_scalar('perf/test_acc5_woema', test_stats_woema['acc5'], epoch)
            log_writer.add_scalar('perf/test_loss_woema', test_stats_woema['loss'], epoch)
