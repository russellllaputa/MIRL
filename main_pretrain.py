...
def main(args):
    ...
    # define the model
    model = models_mirl.__dict__[args.model](norm_pix_loss=args.norm_pix_loss)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        misc.save_model_latest(args=args, model=model, model_without_ddp=model_without_ddp, optimizer=optimizer, loss_scaler=loss_scaler, epoch=epoch)
        ...
        if args.output_dir and misc.is_main_process():
            if log_writer is not None:
                for k in log_stats:
                    if k not in ['lr', 'epoch']:
                        log_writer.add_scalar(f'epoch/{k}', log_stats[k], epoch)
                log_writer.flush()
