...
class CrossAttention(nn.Layer):
    ...
    def forward(self, x, y):
        N, C = x.shape[1:]
        q = self.q(x).reshape((-1, N, self.num_heads, C // self.num_heads)).transpose((0 ,2 ,1 ,3)) # [B, num_heads, N, head_dim]
        kv = self.kv(y).reshape((-1, y.shape[1], 2, self.num_heads, C // self.num_heads)).transpose((2, 0, 3, 1, 4)) # [2, B, num_heads, N*r, head_dim]
        k, v = kv[0], kv[1]
...
#         if isinstance(norm_layer, str):
#             self.norm_start = eval(norm_layer)(dim, epsilon=epsilon)
#             self.norm_q = nn.LayerList([eval(norm_layer)(dim, epsilon=epsilon) for _ in range(num_cross)])
#             self.norm_k = nn.LayerList([eval(norm_layer)(dim, epsilon=epsilon) for _ in range(num_cross)])
#             self.norm_end = norm_layer(dim)
#         elif isinstance(norm_layer, Callable):
#             self.norm_start = norm_layer(dim)
#             self.norm_q = nn.LayerList([norm_layer(dim) for _ in range(num_cross)])
#             self.norm_k = nn.LayerList([norm_layer(dim) for _ in range(num_cross)])
#             self.norm_end = norm_layer(dim)
#         else:
#             raise TypeError(
#                 "The norm_layer must be str or paddle.nn.layer.Layer class")
...
#             attn_drop=attn_drop,
#             proj_drop=drop)
#         self.cross_attns = nn.LayerList([CrossAttention(
#             dim,
#             num_heads=num_heads,
#             qkv_bias=qkv_bias,
#             qk_scale=qk_scale,
#             attn_drop=attn_drop,
#             proj_drop=drop) for _ in range(self.num_cross)])
#         # NOTE: drop path for stochastic depth, we shall see if this is better than dropout here
#         self.drop_path = DropPath(drop_path) if drop_path > 0. else Identity()
...
#         """ x: the query which is the directe output from the (l-1)th layer
#             keys: could be mutiple keys or a single one from previous blocks
#         """
#         x = x + self.drop_path(self.attn(self.norm_start(x)))
#         for i in range(self.num_cross):
#             x = x + self.drop_path(self.cross_attns[i](self.norm_q[i](x), self.norm_k[i](keys[i])))
#         x = x + self.drop_path(self.mlp(self.norm_end(x)))
#         return x
...
class CrossBlock(nn.Layer):
    def __init__(self,
        ...
        if isinstance(norm_layer, str):
            ...
            self.norm4 = eval(norm_layer)(dim, epsilon=epsilon)
        elif isinstance(norm_layer, Callable):
            self.norm1 = norm_layer(dim)
            self.norm2 = norm_layer(dim)
    ...
    def forward(self, x, y):
        x = x + self.drop_path(self.attn(self.norm1(x)))
        x = x + self.drop_path(self.cross_attn(self.norm2(x), self.norm3(y))) # does y need a norm?
