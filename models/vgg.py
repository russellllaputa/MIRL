...
class VGG(nn.Layer):
    ...
    def __init__(self, features, num_classes=0, with_pool=False):
        super().__init__()
...
def _vgg(arch, cfg, batch_norm, pretrained, **kwargs):
    ...
    if pretrained:
        if arch == 'vgg16':
            param = paddle.load('/bpfs/v2_mnt/VIS_LX/bpfsrw3/face/data/huangguoxi/ckpt/vgg16.pdparams')
            model.load_dict(param)
            print('loaded pretrained weights to VGG16')
        elif arch == 'vgg19':
            pass
        else:
            pass
